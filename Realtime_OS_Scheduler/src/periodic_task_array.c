...
// Functions used of Calculations
...
// If an task executes for 2ms and its period is 10ms then its utilization is 0.2%
...
// as static is used in the variable declaration. Normal local variables (without static) are created on the "stack" every time a function starts, and destroyed when the function ends.
// static variables are different. They are created and initialized in a special part of the computer's memory (the Data Segment) before your program even starts running (before main() is even called).
...
// This function calculates exactly how many individual "jobs" (task executions) will occur during one complete scheduling cycle (the Hyper-period).